...
const NAV = [
  ...
  { id: 'structure', label: 'Structure & Panels' },
  { id: 'scoring', label: 'Scoring' },
  { id: 'devices', label: 'Judge Devices' },
] as const;
      ...
      <div style={{ width: 240, flex: 'none', background: C.greenDeep, color: '#fff', display: 'flex', flexDirection: 'column', padding: '22px 0' }}>
        ...
        {onExit && (
          <div style={{ marginTop: 'auto', padding: '0 22px' }}>
            <div onClick={onExit} style={{ cursor: 'pointer', fontSize: 13, color: '#9DBDB4' }}>← Exit preview</div>
          </div>
        )}
      </div>
      <div style={{ flex: 1, overflow: 'auto', padding: 28 }}>
        {tab === 'leaderboard' ? <Leaderboard /> : <Placeholder label={NAV.find((n) => n.id === tab)?.label ?? ''} />}
      </div>
    </div>
  );
}
...
function Placeholder({ label }: { label: string }) {
  return (
    <div style={{ background: C.cream, borderRadius: 8, padding: 48, textAlign: 'center', color: C.muted, boxShadow: '0 6px 22px rgba(20,40,36,.14)' }}>
      <div style={{ fontFamily: serif, fontSize: 22, color: C.greenDeep, marginBottom: 8 }}>{label}</div>
      <div>Coming next.</div>
    </div>
  );
}
