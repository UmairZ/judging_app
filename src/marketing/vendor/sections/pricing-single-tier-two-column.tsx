...
export function PricingSingleTierTwoColumn({
  ...
  return (
    <section className={clsx('py-16', className)} {...props}>
      <Container>
        <div className="grid grid-cols-1 gap-x-2 rounded-xl bg-taupe-950/2.5 p-2 lg:grid-cols-2 dark:bg-white/5">
          ...
          <div className="rounded-sm bg-taupe-100 p-6 sm:p-10 dark:bg-taupe-950">
            <div className="flex items-baseline gap-2">
              <p className="text-[5rem]/24 font-light tracking-tight text-taupe-950 sm:text-8xl/32 dark:text-white">
                {price}
              </p>
            ...
            <ul className="mt-6 grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-1">
              {features.map((feature, index) => (
                <li key={index} className="flex gap-3 text-sm/5">
                  <span className="flex size-5 shrink-0 items-center justify-center rounded-xs bg-taupe-950 dark:bg-taupe-700">
                    <CheckmarkIcon className="size-3 stroke-white" />
                  </span>
                  <p className="text-taupe-700 dark:text-taupe-400">{feature}</p>
                </li>
              ))}
            </ul>
          </div>
